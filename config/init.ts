...
const init = async (): Promise<void> => {
  try {
    ...
    if (['development', 'production', 'test'].includes(process.env.NODE_ENV)) {
      ...
      config.oauth2.authorizationURL = (process.env.FORGE_AUTH_URL ? process.env.FORGE_AUTH_URL: config.oauth2.authorizationURL);
      ...
      config.oauth2.tokenURL = (process.env.FORGE_TOKEN_URL ? process.env.FORGE_TOKEN_URL: config.oauth2.tokenURL);
      ...
      config.userprofile_path = (process.env.USERPROFILE_PATH ? process.env.USERPROFILE_PATH: '/userprofile/v1/');
      config.logoutaccount_url = (process.env.LOGOUTACCOUNT_URL ? process.env.LOGOUTACCOUNT_URL: config.logoutaccount_url);
